...
import {
  ...
  fetchGoogleRelatedQueries,
} from "../_trends/google-trends.ts";
import {
  AuthError,
  corsHeaders,
  errorToResponse,
...
const EXPECTED_TEMPLATE_PAGE_NAME =
  (Deno.env.get("TEMPLATE_PAGE_NAME") || "트렌드 콘텐츠 발견기").trim();
...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case "cancel-search":
        return await handleCancelSearch(req);
      case "trend-daily":
...
async function handleEnsureSearchDatabase(req: Request): Promise<Response> {
  ...
  const body = await req.json();
  const { user_id } = body;
  const requestedPageId = typeof body.page_id === "string"
    ? body.page_id.trim()
    : "";
...
async function handleListDatabases(req: Request): Promise<Response> {
  ...
  const databases = data.results.map((db: any) => {
    ...
    const is_snappy = props["키워드"]?.type === "title" &&
      "매체" in props &&
      "상태" in props;
    return {
      id: db.id.replace(/-/g, ""),
...
async function handleVerifyUser(url: URL): Promise<Response> {
  ...
  const { data, error } = await getSupabase()
    .from("users")
    .select(
      "id, subscription_tier, notion_api_key_encrypted, notion_database_id",
    )
    .eq("id", user_id)
    .single();
...
async function handleHistory(_req: Request, url: URL): Promise<Response> {
  ...
  const { data, error } = await getSupabase()
    .from("search_logs")
    .select(
      "keyword, platforms, period, result_count, duration_ms, status, error_message, created_at",
    )
    .eq("user_id", user_id)
...
async function handleAdminListUsers(req: Request, url: URL): Promise<Response> {
  ...
  const { data, error, count } = await getSupabase()
    .from("users")
    .select(
      "id, email, subscription_tier, subscription_expires_at, created_at",
      { count: "exact" },
    )
    .order("created_at", { ascending: false })
...
async function handleAdminUpgradeUser(req: Request): Promise<Response> {
  ...
  if (!["free", "light", "standard", "premium"].includes(subscription_tier)) {
    throw new ValidationError(
      `Invalid subscription_tier: ${subscription_tier}`,
    );
  }
  ...
  logger.info("Admin upgraded user", {
    user_id,
    subscription_tier,
...
async function handleGetSearchStatus(url: URL): Promise<Response> {
  ...
  const { data, error } = await getSupabase()
    .from("users")
    .select(
      "searching_since, search_progress, search_cancel_requested_at, last_search_error, last_related_keywords",
    )
    .eq("id", user_id)
    .single();
  ...
  return jsonResponse({
    ...
    cancel_requested: data.search_cancel_requested_at !== null,
...
// === 진행 중인 검색 취소 요청 ===
async function handleCancelSearch(req: Request): Promise<Response> {
  if (req.method !== "POST") throw new ValidationError("POST required");
  const { user_id } = await req.json();
  if (!user_id || typeof user_id !== "string") {
    throw new ValidationError("user_id required");
  ...
  const { data: user, error } = await getSupabase()
    .from("users")
    .select("searching_since, notion_api_key_encrypted, notion_database_id")
    .eq("id", user_id)
    .single();

  if (error || !user) throw new AuthError("User not found");
  if (!user.searching_since) {
    return jsonResponse({
      success: true,
      searching: false,
      message: "진행 중인 검색이 없습니다.",
    });
  }

  await requestSearchCancel(user_id);
  ...
  if (user.notion_api_key_encrypted && user.notion_database_id) {
    try {
      ...
      logger.warn(
        "Failed to clear search embed status during cancel (non-fatal)",
        {
          user_id,
...
async function handleSetupSearchButton(req: Request): Promise<Response> {
  ...
  const apiKey = await decryptNotionKey(user.notion_api_key_encrypted);
  const notion = new NotionClient(apiKey);
  const embedUrl = await notion.updateSearchEmbed(
